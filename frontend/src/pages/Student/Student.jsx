      ...
      try {
        ...
        if (data) {
          ...
          setDepartment(data.department);
            ...
            const tempProfilePictureUrl = await checkImage(`${baseUrl}.JPG`) || await checkImage(`${baseUrl}.jpg`) 
  ...
  return (
    <div className={studentStyle.fullPageContainer}>
      ...
      <div className={studentStyle.studentDetails}>
        ...
        <h3 className={studentStyle.studentId}>Registration Number: {registrationNumber}</h3>
        <h3 className={studentStyle.department}>Department: {department}</h3>
        <h3 className={studentStyle.email}>E-mail: <a href={`mailto:${sMail}`}>{sMail}</a></h3>
      </div>
    </div>
  );
}
