...
function Course() {
    ...
    const handleRleatedProjectsClick = useCallback((projectId) => {
    ...
    return (
        <div className={courseStyles.courseContainer}>
            <div className={courseStyles.courseDetails}>
                <div className={courseStyles.title}><strong>Course Name: </strong>{course.title}</div>
                <div className={courseStyles.courseCode}><strong>Course Code:</strong> {course.courseCode}</div>
                <div className={courseStyles.description}><strong>Description:</strong> {course.description}</div>
            </div>
            <section className={courseStyles.relatedProjectsSection}>
                    ...
                    {error && (
                      <div className={courseStyles.error}>{error}</div>
                    )}
                      ...
                      <div className={courseStyles.projectGrid}>
                        {relatedProjects.map((project) => (
                          <article
                            ...
                            role="link"
                            tabIndex={0}
                            onKeyDown={(e) => e.key === 'Enter' && handleRleatedProjectsClick(project.id)}
                          >
                            <div
                              className={courseStyles.projectCoverImage}
                              style={{ backgroundImage: `url(${project.coverImageUrl || 'https://placehold.co/300X300'})` }}
                              aria-label={`${project.title} preview`}
                            />
                            <h3 className={courseStyles.projectTitle}>{project.name} <div className={courseStyles.tinyId}>({project.id})</div></h3>
                          </article>
                        ))}
                      </div>
                    )}
                    ...
                    {!loading && !error && relatedProjects.length === 0 && (
                      <div className={courseStyles.noRelatedProjects}>No related projects available at the moment</div>
                    )}
                  </section>
        </div>
    );
