    ...
    if (process.env.USE_LOCAL_DATA === 'true' || !GITHUB_TOKEN) {
      if (localPath) {
        try {
          ...
          throw new Error(`Failed to read local data file: ${localError.message}`);
        }
      }
    } else {
      ...
      if (localPath) {
        try {
          ...
          console.log(`Could not read from local file (${localError.message}), falling back to GitHub`);
...
app.get('/courses/:courseId', async (req, res) => {
  try {
    ...
    const course = courses.find(c => c.courseCode.toLowerCase() === courseId.toLowerCase());
    if (!course) {
...
app.get('/instructor/:instructorName', async (req, res) => {
  try {
    ...
    const instructor = instructors.find(i => 
      i.name.toLowerCase().replace(/(\s|%20)/g, '-') === instructorName.toLowerCase().replace(/(\s|%20)/g, '-')
    );
    if (!instructor) {
...
app.get('/projects/id/:projectId/n/:projectName', async (req, res) => {
  try {
    ...
    if (!projectId || !projectName || typeof projectId !== 'string' || typeof projectName !== 'string') {
      return res.status(400).json({ error: 'Invalid project ID or name' });
    }

    const projects = await fetchData(PROJECTS_URL);
    const normalizedProjectName = projectName.toLowerCase().replace(/(\s|%20)/g, '-');
    const project = projects.find(p => 
      p.id.toLowerCase() === projectId.toLowerCase() && 
      p.name.toLowerCase().replace(/(\s|%20)/g, '-') === normalizedProjectName
    );
    if (!project) {
      return res.status(404).json({ error: 'Project not found' });
    }
...
app.get('/student/:studentId', async (req, res) => {
  try {
    ...
    const student = students.find(s => s.sNumber.toLowerCase() === studentId.toLowerCase());
    if (!student) {
